...
Lambda C
============
hands-on guide on simple lambda expressions
...
### 표기에 관하여 (notation convention)

앞선 설명에서 사용한 `=α=>`, `=β=>`은 각각 α변환, β축약을 사용하여 식을 변형했다는 의미이다. 비슷하게 `===>`는 아무 변형 없이 공백을 없애거나, alias를 확장했다는 의미로 사용된다. `≡`라는 표기는 양변의 람다식이 임의 갯수의 변형을 거쳐 같은 식이 된다는 뜻이다. 수학에서 `=`유사한 용도로 사용된다.

설명의 과정에서 `:=` 기호를 사용하여 람다식의 alias를 정의할 것이다. 예를 들어
```
expr0 :- λx. x y
expr1 := λx. x (expr0 (λz. z))
expr2 := expr1 a (λb. expr1)
```
이런 식으로. 이것은 단지 식을 간략히 표현하여 이해를 돕기 위해 사용될 뿐, 람다대수의 문법에 해당하지 않는다. 어떤 람다식에 alias를 사용한다면 그 식은 유한한 단계의 alias 확장을 통해 순수 람다식으로 환원될 수 있어야 한다.
```
expr2 := expr1 a (λb. expr1)
       ≡ (λx. x (expr0 (λz. z))) a (λb. (λx. x (expr0 (λz. z))))
       ≡ (λx. x ((λx. x y) (λz. z))) a (λb. (λx. x ((λx. x y) (λz. z))))
```
다시 말해, `:=`를 사용해서 재귀식을 정의하거나 할 수는 없다.
```
# 유효하지 않은 정의
rec := λx. x rec
     ≡ λx. x (λx. x rec)
...
몇몇 표현식(함수)들은 그 간단함에도 불구하고 굉장히 유용해서 이름으로 불린다. 앞으로 설명에서 간혹 사용하니 미리 알아두자.

```
id := λx. x
```
인자로 받은것을 그대로 리턴한다. 사실상 있으나 마나인 식인데, 아무 일도 하지 않고 자리를 채울 필요가 있을때 요긴하게 사용된다.

```
const := λx y. x
```
두 번째로 받는 인수는 무시하고 첫번째 인수를 그대로 리턴한다. `const a`라는 함수는 인수를 어떻게 주던 간에 항상 `a`를 리턴하기때문에 constant라는 이름이 붙었다.
...
이를 이용하면 처치식 자연수 3은 `S (S (S Z))` 이렇게 정의된다. 여기에 β축약을 사용하면 처음 만들었던 정의를 얻을 수 있다.
```
     S (S ( S                                              Z            ))
...
마지막으로 자연수 n을 받아서 처치식 자연수를 반환하는 컨버터를 만들어보자. 이 함수는 도우미 함수이니 람다식으로 환원될 필요도 없고 그럴 수도 없다.
```python
def n2c(n):
  pass
...
보통의 뺄셈은 음수를 반환할 경우가 있기 때문에 정수집합의 연산으로 정의된다. 우리는 아직 정수를 정의하지 않았으니 음수가 나오지 않는 자연수 뺄셈을 대신 구현하기로 한다. 자연수 뺄셈은 두 자연수를 받아 뺀 결과가 음수가 된다면 그냥 0을 반환하는 연산이다.
```python
# 자연수 뺄셈을 설명하는 파이썬 코드. 앞으로 구현할 방법과는 상관없다.
def nat_minus(a, b):
    return 0 if a < b else a - b
```

처치식 자연수 `n`은 일변수 함수를 받아 n번 반복적용하는 함수로 바꿔준다고 했다. 그럼 자연수 뺄셈은 `n`과 `m`을 받아 '일변수 함수를 n-m번 반복적용하는 함수'로 바꿔주어야 한다. 하지만 이미 n번 적용해버린 함수를 어떻게 m번 되물릴 수 있을까. 도대체 감이 잘 잡히지 않는다. 이럴 때는 문제를 작게 만드는것이 좋다.

먼저 가장 간단한 뺄셈함수인 `pred`를 정의해보자. `pred`는 말하자면 `succ`의 역함수로, 자연수 하나를 받아 그 이전 자연수 (혹은 0)을 반환한다.
```python
# 마찬가지로 predecessor 함수를 설명하는 파이썬 코드. 앞으로 구현할 방법과는 상관없다.
def pred(a):
  return nat_minux(a, 1)
```

람다식에서 `pred`를 구현하기란 쉽지 않다. 학구열에 넘쳐 직접 정의해보고 싶은 사람은 말리지 않겠지만, 보편적으로 알려진 정의는 지금까지 본 식중에 어렵기로 세 손가락 안에 들 만큼 복잡하다.
```
P := λn f x. n (λg h. h (g f)) (λu. x) (λu. u)
   = λn f x. n (λg h. h (g f)) (const x) id
```
`P`가 정말로 `predecessor`로써 작동하는지 미심쩍다면 직접 `P Z`, `P (S Z)`, `P (S (S Z))`를 전개해보자. 신묘하게도 `Z`, `Z`, `S Z`으로 적절히 정리된다.

구체적으로 어떻게 작동하는지 식을 분석해보자. 중요한 부분은 람다식 몸체의 `n (λg h. h (g f)) (const x)`부분이다. `P (S n)`을 확장할때 해당 부분은 이런 패턴으로 변화한다.
```
let (f>>.>>.) = \x y -> f >> x >> y
let a >> b = b a
let cx = const x
...
≡ f >> (n (f>>.>>.) cx) >> .
```
이를 general한 형태에 적용시키면 이렇게 삼엽충같이 생긴 풀이가 된다.
```
let a = S b = S (S c) = S (S (S d))
...
P a ≡ λf x. a (f>>.>>.) cx id
    ...
    ≡ b
```
`b`와 동치라는 결과를 얻을 수 있다.

`P`의 구현을 적당히 이해했으면 이제 자연수 뺄셈을 구현할 차례이다. `pred`를 적절히 사용해서 자연수 뺄셈을 정의하라. _currying 함수들이 필요할수도 있다._
```
SUB :=
```
...
그렇다면 브랜칭을 함수로 어떻게 나타낼까. 부작용(side effect)이 없는 연산만 [1] 사용한다면 if-else 구문은 삼항연산자를 사용하는것과 같다. 삼항 연산자는 람다식으로 이렇게 표현된다.
```
true  := λx. λy. x ≡ λx y. x
false := λx. λy. y ≡ λx y. y
```
불리언 값은 인자 2개를 받는 함수인데 True는 첫번째 인자를, False는 두 번째 인자를 반환한다.[2] x가 불리언 타입이라고 한다면 x a b 는 x가 true일때 a, false일때 b가 된다.
...
이제 다음의 연산들을 구현하라. 그리고 check_b+ 함수들을 사용해 옳은 답이 나오는지 확인해보라.
```python
def neg(b):
  pass
...
----------
[1]: 부작용이 없는 함수는 함수형 언어에서 pure한 함수라고 불린다. 일부 소수의 함수형 언어들만이 pure한 함수로만 이루어졌있다. c의 함수는 local static 변수나 전역 변수를 가질 수 있어 많은 경우 TODO
[2]: 그렇다! `true`는 `const`랑 똑같이 생겼다!
...
다음 함수를 사용하면 람다식 튜플[1]을 파이썬 튜플로 변환할 수 있다.
```python
def c2p(p):
  return fst(p), snd(p)
```
...
----------
[1]: 계속 '처치식'이라는 표현을 쓰다가 람다식으로 바꾸었는데, 자연수와 불리언을 제외하고는 처치가 직접 정의했는지 (본인은) 모르기 때문이다. 표현만 다르고 같은 의미이다. 헷갈리지 말자.
...
----------
[12]: 다시 말하지만 의미는 실제로 어떻게 생겼는지와 관계없이 조작규칙에 의해 부여되는 것이다. 튜플의 조작규칙에 의해 (3, 4)와 (5, 6)이 동일하게 취급된다면 적힌 기호(숫자)의 다른점은 상관이 없다.
