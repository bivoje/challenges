...
알론조 처치는 람다대수에서 자연수 n을 '함수 f를 인자로 받아 f를 n번 반복 적용하는 함수 fn을 리턴하는' 함수로 표현할 수 있음을 발견했다. 예를들어 자연수 1과 3을 one, three라는 함수로 나타낸다고 하면 다음과 같이 작동할 것이다. 구현은 신경쓰지 말고 one과 three가 기능적으로 어떻게 행동하는지를 관찰하라.
```python
def one(f):
  pass
...
```
여기서 `append_pip`이란 함수는 람다식으로 환원될 수 없지만 큰 문제는 없다. 중요한건 `three`가 임의의 함수를 인자로 받아 세번 연속적용하는 함수를 리턴해준다는 것이지 인자로 주는 함수가 무엇인가가 아니다. 반면 `one`과 `three`는 람다식으로 환원될 수 있도록 구현되어야 한다.
